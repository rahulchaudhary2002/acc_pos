    ...
    document: StatefulBuilder(
      builder: (context, setState) {
        ...
        Future<void> markPrinted() async {
          if (context.mounted) {
            await context.read<PosService>().recordPurchaseReturnPrint(result.documentId);
            setState(() => printCount++);
          }
        }
          ...
          actions: [
              ...
              onPressed: () => showPrintMethodSheet(
                ...
                onPdfPrint: () async {
                  await Printing.layoutPdf(
                    onLayout: (_) => buildPdf(),
                    name: 'PurchaseReturn-${result.billNo ?? result.documentNo}',
                  );
                  await markPrinted();
                },
              ),
            ...
            ElevatedButton.icon(
              onPressed: () async => Printing.sharePdf(
                ...
                filename: 'PurchaseReturn-${result.billNo ?? result.documentNo}.pdf',
              ),
              style: AppButtonStyles.filled(AppColors.info).copyWith(
                padding: const WidgetStatePropertyAll(EdgeInsets.symmetric(horizontal: 20)),
              ),
