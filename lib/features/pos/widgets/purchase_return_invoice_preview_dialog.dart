...
/// Post-purchase-return receipt — same layout as [showPurchaseInvoicePreview]
/// (see purchase_invoice_preview_dialog.dart), titled "PURCHASE RETURN" and
/// without vendor-invoice/MRN fields, since a return doesn't carry those.
Future<void> showPurchaseReturnInvoicePreview(
  ...
  final metaRows = [
    [('Return No', result.billNo ?? result.documentNo), ('Return Date', _formatDate(now))],
    [('Counter No.', counterNo), null],
    [('Vendor Name', vendorName), null],
  ...
  return showTaxInvoiceDialog(
    ...
    document: TaxInvoiceDocument(
      ...
      actions: [
        ...
        ElevatedButton.icon(
          onPressed: () async => Printing.layoutPdf(onLayout: (_) => buildPdf(), name: 'PurchaseReturn-${result.billNo ?? result.documentNo}'),
        ...
        ElevatedButton.icon(
          onPressed: () async => Printing.sharePdf(bytes: await buildPdf(), filename: 'PurchaseReturn-${result.billNo ?? result.documentNo}.pdf'),
