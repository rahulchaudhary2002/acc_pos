    ...
    document: StatefulBuilder(
      builder: (context, setState) {
        ...
        Future<void> markPrinted() async {
          if (context.mounted) {
            await context.read<PosService>().recordPurchaseBillPrint(result.documentId);
            setState(() => printCount++);
          }
        }
          ...
          actions: [
            ElevatedButton.icon(
              onPressed: () => showPrintMethodSheet(
                ...
                onPdfPrint: () async {
                  await Printing.layoutPdf(onLayout: (_) => buildPdf(), name: 'Purchase-${result.billNo ?? result.documentNo}');
                  await markPrinted();
                },
              ),
            ...
            ElevatedButton.icon(
              onPressed: () async => Printing.sharePdf(
                ...
                filename: 'Purchase-${result.billNo ?? result.documentNo}.pdf',
              ),
              style: AppButtonStyles.filled(AppColors.info).copyWith(
                padding: const WidgetStatePropertyAll(EdgeInsets.symmetric(horizontal: 20)),
              ),
